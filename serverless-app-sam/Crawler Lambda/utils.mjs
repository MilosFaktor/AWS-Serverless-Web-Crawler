...
const tableName = process.env.VISITED_TABLE_NAME;
...
export async function markAllVisited(targets, runId, sourceUrl, rootUrl) {
    ...
    await ddbClient.send(new BatchWriteItemCommand({ RequestItems: { [tableName]: putRequests } }));
}
...
export async function batchGetItems(urls, runId) {
    ...
    try {
        ...
        const results = response.Responses?.[tableName]?.map(item => item.visitedURL.S) || [];
